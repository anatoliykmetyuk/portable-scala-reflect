...
trait ReflectCommons {
  ...
  protected def isModuleClass(clazz: Class[_]): Boolean = {
    try {
  ...
  protected def isInstantiatableClass(clazz: Class[_]): Boolean = {
  ...
  protected def load(fqcn: String, loader: ClassLoader): Option[Class[_]] = {
    try {
  ...
  protected def inheritsAnnotation(clazz: Class[_]): Boolean = {
